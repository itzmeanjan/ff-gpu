...
sycl::event
compute_matrix_matrix_multiplication(sycl::queue& q,
                                     buf_2d_u64_t& mat_a,
                                     buf_2d_u64_t& mat_b,
                                     buf_2d_u64_t& mat_c,
                                     const uint64_t dim,
...
void
check_ntt_correctness(sycl::queue& q,
                      const uint64_t dim,
                      const uint64_t wg_size)
{
...

  {
    ...
    compute_matrix_matrix_multiplication(
...
void
prepare_random_vector(uint64_t* const vec, const uint64_t size)
{
...
void
check_ntt_forward_inverse_transform(sycl::queue& q,
                                    const uint64_t dim,
                                    const uint64_t wg_size)
{
  assert((dim & (dim - 1ul)) == 0);
...
void
check_cooley_tukey_ntt(sycl::queue& q,
                       const uint64_t dim,
                       const uint64_t wg_size)
{
  assert((dim & (dim - 1ul)) == 0);
...
void
check_matrix_transposition(sycl::queue& q,
                           const uint64_t dim,
                           const uint64_t wg_size)
{
...
void
test_compute_twiddles(sycl::queue& q,
                      const uint64_t dim,
                      const uint64_t wg_size)
{
  assert((dim & (dim - 1ul)) == 0);
...
void
test_twiddle_factor_multiplication(sycl::queue& q,
                                   const uint64_t n1,
                                   const uint64_t n2,
                                   const uint64_t wg_size)
{
  ...
  sycl::event evt_3 = twiddle_multiplication(
    q, vec_d, twiddles, n2, n1, n, wg_size, { evt_0, evt_2 });
...

  {
    ...
    sycl::event evt_4 = q.submit([&](sycl::handler& h) {
      ...
      h.depends_on({ evt_3 });
      h.parallel_for(
        sycl::nd_range<2>{ sycl::range<2>{ n, n },
                           sycl::range<2>{ 1, wg_size } },
        [=](sycl::nd_item<2> it) {
          const uint64_t r = it.get_global_id(0);
          const uint64_t c = it.get_global_id(1);
          ...
          corr_ref.fetch_add(
            *(vec_d + r * n + c) % MOD ==
                ff_p_mult(*(vec_s + r * n + c), ff_p_pow(*omega, r * c)) % MOD
              ? 0
              : 1);
        });
    });
...
void
test_six_step_fft(sycl::queue& q, const uint64_t dim, const uint64_t wg_size)
{
  assert((dim & (dim - 1ul)) == 0);
  uint64_t log_2_dim = (uint64_t)sycl::log2((float)dim);
  assert(log_2_dim > 0 && log_2_dim <= TWO_ADICITY);
...
void
test_six_step_ifft(sycl::queue& q, const uint64_t dim, const uint64_t wg_size)
{
  assert((dim & (dim - 1ul)) == 0);
  uint64_t log_2_dim = (uint64_t)sycl::log2((float)dim);
  assert(log_2_dim > 0 && log_2_dim <= TWO_ADICITY);
...
void
test_six_step_fft_and_ifft(sycl::queue& q,
                           const uint64_t dim,
                           const uint64_t wg_size)
{
  assert((dim & (dim - 1ul)) == 0);
